...
layui.define(['layer', 'laytpl', 'form', 'upload', 'util'], function(exports){
  ...
  if(device.ie && device.ie < 8){
    layer.alert('如果您非得使用ie浏览zsCat社区，那么请使用ie8+');
  }
  ...
  var gather = {
    ...
    ,layEditor: function(options){
        ...
        picture: function(editor){ //插入图片
          layer.open({
            ...
            ,success: function(layero, index){
              ...
              layui.upload({
                url: '/api/upload/'
                ,elem: '#zsCat-jie-upload .layui-upload-file'
                ,success: function(res){
                  if(res.status == 0){
                    image.val(res.url);
                  } else {
      ...
      layui.use('face', function(face){
        ...
        $(options.elem).each(function(index){
          ...
          parent.prepend(html);
          parent.find('.zsCat-edit span').on('click', function(event){
    ...
    ,content: function(content){
      ...
      }).replace(/@(\S+)(\s+?|$)/g, '@<a href="javascript:;" class="zsCat-aite">$1</a>$2') //转义@
  ...
  //点击@
  $('body').on('click', '.zsCat-aite', function(){
