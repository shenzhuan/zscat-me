	...
	view = {
		...
		expandCollapseNode: function(setting, node, expandFlag, animateFlag, callback) {
			...
			if (node.isParent) {
				...
				if (node.open) {
					...
					if(undefined == node.icon || "" == node.icon || node.icon.indexOf("fa") == -1){
		...
		makeNodeIcoClass: function(setting, node) {
			...
			if (!node.isAjaxing) {
				...
				if(undefined == icon || "" == icon || icon.indexOf("fa") == -1){
					if (node.isParent) {
					...
					icoCss.push(icon);
					return consts.className.BUTTON + " cusicon ace-icon " + icoCss.join(' ');
				}
