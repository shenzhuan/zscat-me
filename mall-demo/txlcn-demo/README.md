...
执行`mall-doc\sql\tx-manager.sql`和`mall-doc\sql\tx_logger.sql`脚本

### 2. 修改TxManager的配置

工程目录：`mall-transaction\txlcn-tm`，修改nacos、数据库和redis

### 2. 启动TxManager

工程目录：`mall-transaction\txlcn-tm`

### 3. 初始化demo的数据

执行`mall-demo\txlcn-demo\txlcn-demo.sql`脚本
